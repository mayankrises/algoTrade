...
def run_backtest(df: pd.DataFrame, capital: float, ticker_name: str = "RELIANCE.NS"):
    ...
    for idx, row in df_signals.iterrows():
        ...
        if signal == 1 and position is None:
            # BUY signal - use full capital for simplicity
    ...
    # Force close open position at the end of the backtest to evaluate terminal capital
    ...
    # Format chartData for Frontend (standardised structure)
    chart_data = []
    for idx, row in df_signals.iterrows():
