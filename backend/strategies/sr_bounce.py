...
def generate_signals(df: pd.DataFrame):
    ...
    # Calculate Support & Resistance as 20-period rolling low and high (shifted by 1 to avoid lookahead)
    ...
    # Generate signals:
    # Buy if close is near support and starts moving up (or crosses above support)
    # Sell if close is near resistance and starts moving down (or crosses below resistance)
    for i in range(1, len(df)):
        prev_close = df[close_col].iloc[i-1]
        curr_close = df[close_col].iloc[i]
        ...
        # Buy if we cross above support or bounce off support
        if prev_close <= support and curr_close > support:
            df.loc[df.index[i], 'signal'] = 1
        # Sell if we cross below resistance or bounce off resistance
        elif prev_close >= resistance and curr_close < resistance:
            df.loc[df.index[i], 'signal'] = -1
    ...
    for idx, row in df_signals.iterrows():
        ...
        if signal == 1 and position is None:
            ...
            trades.append({
                ...
                "exit_reason": "Resistance Bounce Exit"
            })
            position = None
